import {exec, log, logError, logWarn, newSQLError, READ_ONLY_REGEX, txLocks} from './common'
...
export class SQLitePluginTransaction {
    ...
    if (!!values && values.constructor === Array) {
      for (let j = 0, len1 = values.length; j < len1; j++) {
        ...
        if (v === null || v === void 0 || t === 'number' || t === 'string') {
          ...
          logWarn('addStatement - parameter of type <' + t + '> converted to string using toString()')
        } else {
  ...
  run = () => {
    ...
    exec("backgroundExecuteSqlBatch", {
      dbargs: {
